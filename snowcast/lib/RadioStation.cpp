...
RadioStation::RadioStation(
    ...
    : m_stationFiles{stationFiles}, m_startTime{startTime},
      m_tickDuration{tickDuration}, m_bytesPerTick{bytesPerTick} {}

size_t RadioStation::currByteCount() const {
  const auto ticksExecuted =
      ((std::chrono::system_clock::now() - m_startTime) / m_tickDuration);
  return ticksExecuted * m_bytesPerTick;
}

bool RadioStation::active() const { return activeSong.has_value(); }

const std::string &RadioStation::currSong() const { return activeSongName; }
